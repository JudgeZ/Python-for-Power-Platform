...
from ..models.power_platform import EnvironmentSummary, PowerApp, CloudFlow, FlowRun
...
class PowerPlatformClient:
    ...
    def list_environments(self) -> List[EnvironmentSummary]:
        resp = self.http.get("environmentmanagement/environments", params={"api-version": self.api_version})
        data = resp.json()
    ...
    def get_environment(self, environment_id: str) -> EnvironmentSummary:
        resp = self.http.get(f"environmentmanagement/environments/{environment_id}", params={"api-version": self.api_version})
        return EnvironmentSummary.model_validate(resp.json())

    def delete_environment(self, environment_id: str, validate_only: Optional[bool] = None) -> None:
    ...
    def list_environment_settings(self, environment_id: str) -> Dict[str, Any]:
        resp = self.http.get(f"environmentmanagement/environments/{environment_id}/settings", params={"api-version": self.api_version})
        return resp.json()

    def upsert_environment_setting(self, environment_id: str, body: Dict[str, Any]) -> None:
        self.http.post(f"environmentmanagement/environments/{environment_id}/settings", params={"api-version": self.api_version}, json=body)

    def list_apps(self, environment_id: str, top: Optional[int] = None, skiptoken: Optional[str] = None) -> List[PowerApp]:
    ...
    def list_cloud_flows(self, environment_id: str, **filters: Any) -> List[CloudFlow]:
        ...
        resp = self.http.get(f"powerautomate/environments/{environment_id}/cloudFlows", params=params)
