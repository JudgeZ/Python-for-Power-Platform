...
* `annotations` honors an optional `top` limit (default: 50 notes per web file), automatically follows `@odata.nextLink` pointers to fetch every page of notes, and surfaces records missing `documentbody` in the manifest as skipped entries so you can backfill the note content.
