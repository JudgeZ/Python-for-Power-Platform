    ...
    config = ConfigData(
        ...
        profiles={
            "default": Profile(
                ...
                secret_backend="keyring",  # nosec B106  # noqa: S106
                # Bandit B106: pointer to stored secret.
                secret_ref="svc:user",  # nosec B106  # noqa: S106
            )
        },
    )
...
def test_resolve_token_getter_legacy_profile_defaults_to_device_flow(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PACX_ACCESS_TOKEN", raising=False)
